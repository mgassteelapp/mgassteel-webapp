...
function AssistantTab({ prices, scenarios, gsStatus, session }) {
  ...
  const calcResult = selectedProduct && parseFloat(calcQty) > 0 ? (() => {
    ...
    const costN   = parseFloat(p.costFloor)   || 0;
    const marginU = parseFloat(p.marginRetail) || 0;
    // Column U may be stored as decimal (0.095) or percentage (9.5)
    // Normalise to percentage for comparison
    const marginUPct  = marginU > 0 && marginU < 1 ? marginU * 100 : marginU;
    const isLowMargin = marginUPct > 0 && marginUPct < LOW_MARGIN_THRESHOLD;
    const minFloor = costN > 0 ? costN * (1 + MIN_MARGIN) : priceE;
    let recPrice, tierLabel;
              ...
              : (
                <div style={{ border:`1px solid ${C.border}`, borderRadius:10, overflow:"hidden" }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr 1fr 1fr 1fr", background:C.navy, padding:"7px 12px", gap:8 }}>
                    {["Kod","Produk / Saiz","Retail (RM)","Kuantiti (RM)","Kredit (RM)"].map(h=>(
                      <div key={h} style={{ color:C.white, fontSize:10, fontWeight:700, textTransform:"uppercase" }}>{h}</div>
                    ))}
                  </div>
                  {/* Rows */}
                  {codeResults.slice(0,10).map((p,i)=>(
                    <div key={p.id} style={{ display:"grid", gridTemplateColumns:"1fr 2fr 1fr 1fr 1fr", padding:"9px 12px", gap:8, background:i%2===0?C.white:C.gray, borderBottom:`1px solid ${C.border}`, alignItems:"center" }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:10, color:C.muted }}>{p.size||""} {p.grade?"| "+p.grade:""} | {p.category}</div>
                      </div>
                  ...
                  {codeResults.length>10 && (
                    <div style={{ padding:"8px 12px", background:C.gray, fontSize:11, color:C.muted, textAlign:"center" }}>
                      {codeResults.length-10} produk lagi — taip lebih spesifik untuk kecilkan carian
                    </div>
                  )}
                </div>
              )
                  ...
                  <div style={{ background:C.navy, borderRadius:10, padding:"16px 18px", marginBottom:12 }}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                      <div>
                        ...
                        <div style={{ color:"#fcd34d", fontWeight:800, fontSize:30 }}>RM {fmtPrice(calcResult.recPrice, calcResult.cat)}</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ color:"#94a3b8", fontSize:11, marginBottom:2 }}>Jumlah ({calcResult.qty} unit)</div>
                        <div style={{ color:"#86efac", fontWeight:800, fontSize:30 }}>RM {fmtPrice(calcResult.totalPrice, calcResult.cat)}</div>
                      </div>
                    </div>
                  </div>
                  {calcResult.priceF > 0 && (
                    <div style={{ background:"#f5f3ff", border:"1px solid #c4b5fd", borderRadius:8, padding:"12px 14px", marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      ...
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontWeight:800, fontSize:16, color:"#6d28d9" }}>RM {fmtPrice(calcResult.priceF, calcResult.cat)}/unit</div>
