...
function normaliseDesc2(desc2) {
  ...
  s = s.replace(/\bWARNA\b\s*/g, '');
  s = s.replace(/(\d)(KAKI|FEET|FOOT|METER|METRE|MM|CM)/g, '$1 $2');
  s = s.replace(/(\d)\s*M\b(?!M)/g, '$1 M');
  s = s.replace(/\s*-\s*/g, ' ');
...
function buildCSV(exceptions, matchedRows, matchedDoRows, stockNoSales, userName, reportDate) {
  ...
  const exHdr = ['"Status"','"Kod Item"','"Desc2"','"Pembekal"','"Pelanggan"','"Agen"',
                 '"Tarikh PO"','"Tarikh Jualan"','"No PO"','"No Dok Jualan"',
                 '"Qty PO"','"Qty Jualan"','"Beza"'].join(',');
  const matchHdr = ['"Status"','"Kod Item"','"Desc2"','"Pembekal"','"Pelanggan"','"Agen"',
                    '"Tarikh PO"','"Tarikh Jualan"','"No PO"','"No Dok Jualan"',
      ...
      <div style={{ background:C.white, borderRadius:12, border:`0.5px solid ${C.border}`,
        ...
        {runLog.length > 0 && (
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
            ...
            <tbody>
              {runLog.slice(0, 5).map((r, i) => {
                const t = r.trigger_source || '';
                const who = t === 'cron' ? '🤖 Auto (jadual)'
