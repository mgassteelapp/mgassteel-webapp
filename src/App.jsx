...
function getRulesAnswer(text, prices=[], scenarios=[]) {
  ...
  const priceInfo = matched.length>0
    ? "\n\n**Harga semasa dalam senarai:**\n" + matched.slice(0,3).map(p=>`• ${p.product} ${p.size} (${p.grade}) — RM ${parseFloat(p.price).toFixed(2)} / ${p.unit}`).join("\n")
    : "";
...
function LoginScreen({ onLogin }) {
  ...
  useEffect(() => {
    gsGet("getUsers").then(r => {
      if (r.success && r.users.length > 0) setGsUsers(r.users);
    });
  }, []);

  const tryLogin = async () => {
    if (locked) return;
    // Check Google Sheets users first, fallback to hardcoded STAFF_PINS
    const allUsers = gsUsers.length > 0 ? gsUsers : STAFF_PINS.map(s=>({...s,active:"yes"}));
    const staff = allUsers.find(s => s.name === selected && s.active !== "no");
    if (!staff) { setErr("Sila pilih nama anda atau akaun tidak aktif."); return; }
    if (pin !== staff.pin) {
        ...
        <Card style={{ padding:28 }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <select value={selected} onChange={e=>{setSelected(e.target.value);setErr("");setPin("");}}
              ...
              {(gsUsers.length>0 ? gsUsers.filter(u=>u.active!=="no") : STAFF_PINS).map(s=>(
...
export default function App() {
  ...
  const TABS = [
    ...
    ...(session.role==="owner" ? [{ key:"users", label:"👥 Pengguna" }] : []),
  ];
...
function AssistantTab({ prices, scenarios, gsStatus, session }) {
  ...
  const [calcType, setCalcType] = useState("retail"); // retail | bulk | credit
  ...
  // Tiers: 1-20 = Column D, 21-49 = Column E, 50-99 = dynamic E→N+10%, 100+ = N+10%
  const MIN_MARGIN = 0.10; // 10% minimum margin above cost at 100+ pcs

  const calcResult = selectedProduct && parseFloat(calcQty) > 0 ? (() => {
    const p = selectedProduct;
    const qty = parseFloat(calcQty) || 0;

    const priceD = parseFloat(p.retailPrice || p.price) || 0; // Column D — Retail
    const priceE = parseFloat(p.bulkPrice)   || 0;            // Column E — Competitive
    const priceF = parseFloat(p.creditPrice) || 0;            // Column F — Credit
    const costN  = parseFloat(p.costFloor)   || 0;            // Column N — Cost floor

    const minFloor = costN > 0 ? costN * (1 + MIN_MARGIN) : priceE; // N + 10%

    // Determine recommended price based on quantity
    let recPrice, tier, tierLabel;

    if (qty <= 20) {
      recPrice  = priceD;
      tier      = "D";
      tierLabel = `1–20 pcs → Harga Retail`;
    } else if (qty <= 49) {
      recPrice  = priceE;
      tier      = "E";
      tierLabel = `21–49 pcs → Harga Bersaing`;
    } else if (qty < 100) {
      // Interpolate between E and N+10% as qty goes 50→100
      const ratio    = (qty - 50) / (100 - 50); // 0 at qty=50, 1 at qty=100
      recPrice  = priceE - (ratio * (priceE - minFloor));
      recPrice  = Math.max(recPrice, minFloor); // never below floor
      tier      = "dynamic";
      tierLabel = `${qty} pcs → Harga Dinamik (antara E dan kos+10%)`;
    } else {
      recPrice  = minFloor;
      tier      = "max";
      tierLabel = `100+ pcs → Harga Maksimum Diskaun (kos + 10%)`;
    }

    recPrice = Math.round(recPrice * 100) / 100; // round to 2dp

    const totalPrice   = recPrice * qty;
    const profitPerPc  = costN > 0 ? recPrice - costN : recPrice - priceE;
    const totalProfit  = profitPerPc * qty;
    const marginPct    = costN > 0 ? ((recPrice - costN) / recPrice * 100) : 0;

    // Credit price (fixed, no dynamic)
    const creditTotal  = priceF * qty;

    // Show what happens at different qty milestones
    const qtyScenarios = [10, 20, 30, 40, 50, 60, 70, 80, 100].map(q => {
      let p_;
      if (q <= 20)       p_ = priceD;
      else if (q <= 49)  p_ = priceE;
      else if (q < 100) {
        const r = (q - 50) / 50;
        p_ = Math.max(priceE - (r * (priceE - minFloor)), minFloor);
      } else             p_ = minFloor;
      p_ = Math.round(p_ * 100) / 100;
      return { qty: q, unitPrice: p_, total: Math.round(p_ * q * 100) / 100 };
    });

    return {
      qty, recPrice, tier, tierLabel,
      totalPrice, profitPerPc, totalProfit, marginPct,
      priceD, priceE, priceF, costN, minFloor,
      creditTotal, qtyScenarios,
    };
  })() : null;
  ...
  return (
    <div>
      {scenarios.length>0 && (
        <Alert color="orange">🧠 AI telah dipelajari dengan {scenarios.length} senario tambahan daripada Weelee.</Alert>
      )}
      ...
      <Card style={{ marginBottom:12, padding:"14px 16px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          ...
          <span style={{ fontSize:10, fontWeight:600,
            color: gsStatus==="ok" ? C.green : gsStatus==="error" ? C.red : C.yellow,
          }}>
            {gsStatus==="ok" ? `☁ ${prices.length} produk dari Google Sheets` : gsStatus==="error" ? "⚠️ Google Sheets tidak bersambung" : "⏳ Memuatkan..."}
          </span>
        </div>
        <input
          value={codeSearch}
          onChange={e => setCodeSearch(e.target.value)}
          placeholder="Taip kod produk, nama atau saiz... cth. RHS 2x3, SCH40, MS PIPE"
              ...
              : (
                <div style={{ border:`1px solid ${C.border}`, borderRadius:10, overflow:"hidden" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr 1fr 1fr 1fr", background:C.navy, padding:"7px 12px", gap:8 }}>
                    {["Kod","Produk / Saiz","Retail (RM)","Bersaing (RM)","Kredit (RM)"].map(h=>(
                  ...
                  {codeResults.slice(0,10).map((p,i)=>(
                    <div key={p.id} onClick={()=>{ setSelectedProduct(p); setCalcQty(""); setCodeSearch(""); }}
                      ...
                      <div style={{ fontWeight:800, fontSize:13, color:(p.retailPrice||p.price)>0?C.accent:"#cbd5e1" }}>
                        {(p.retailPrice||p.price)>0?`RM ${parseFloat(p.retailPrice||p.price).toFixed(2)}`:"—"}
                      </div>
                      <div style={{ fontWeight:800, fontSize:13, color:p.bulkPrice>0?C.green:"#cbd5e1" }}>
                        {p.bulkPrice>0?`RM ${parseFloat(p.bulkPrice).toFixed(2)}`:"—"}
                      </div>
                      <div style={{ fontWeight:800, fontSize:13, color:p.creditPrice>0?"#6d28d9":"#cbd5e1" }}>
                        {p.creditPrice>0?`RM ${parseFloat(p.creditPrice).toFixed(2)}`:"—"}
                      </div>
                    </div>
                  ))}
        ...
        {selectedProduct && (
          <Card style={{ marginTop:12, border:`2px solid ${C.accent}` }}>
            <div style={{ background:C.navy, padding:"10px 14px", borderRadius:"12px 12px 0 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                <div style={{ color:C.white, fontWeight:700, fontSize:13 }}>🧮 Kalkulator Harga — {selectedProduct.product}</div>
                <div style={{ color:"#94a3b8", fontSize:11 }}>{selectedProduct.itemCode} | {selectedProduct.category}</div>
              </div>
              <button onClick={()=>setSelectedProduct(null)} style={{ background:"transparent", border:"none", color:"#94a3b8", fontSize:18, cursor:"pointer" }}>×</button>
            </div>
            ...
            <div style={{ padding:16 }}>
              ...
              <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap" }}>
                <div style={{ flex:1, minWidth:120 }}>
                  ...
                  <input type="number" value={calcQty} onChange={e=>setCalcQty(e.target.value)} placeholder="cth. 30"
                    style={{ width:"100%", padding:"9px 11px", borderRadius:8, border:`1.5px solid ${C.border}`, fontSize:16, fontWeight:700, fontFamily:"inherit" }} />
                </div>
                <div style={{ flex:2, minWidth:200, display:"flex", alignItems:"flex-end" }}>
                  <div style={{ background:C.accentLight, border:`1px solid #fcd5a0`, borderRadius:8, padding:"9px 12px", fontSize:11, color:C.accent, fontWeight:600, width:"100%" }}>
                    💡 Harga dikira automatik mengikut kuantiti
                  </div>
                </div>
              </div>
              ...
              {calcResult && (
                <>
                  {/* Main price — clean, staff-facing only */}
                  <div style={{ background:C.navy, borderRadius:10, padding:"16px 18px", marginBottom:12 }}>
                    <div style={{ color:"#94a3b8", fontSize:11, marginBottom:10, textTransform:"uppercase", letterSpacing:0.5 }}>
                      Harga untuk {calcResult.qty} unit
                    </div>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
                      <div>
                        <div style={{ color:"#94a3b8", fontSize:11, marginBottom:2 }}>Harga seunit</div>
                        <div style={{ color:"#fcd34d", fontWeight:800, fontSize:30 }}>RM {calcResult.recPrice.toFixed(2)}</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ color:"#94a3b8", fontSize:11, marginBottom:2 }}>Jumlah ({calcResult.qty} unit)</div>
                        <div style={{ color:"#86efac", fontWeight:800, fontSize:30 }}>RM {calcResult.totalPrice.toFixed(2)}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  {calcResult.priceF > 0 && (
                    <div style={{ background:"#f5f3ff", border:"1px solid #c4b5fd", borderRadius:8, padding:"12px 14px", marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div>
                        ...
                        <div style={{ fontSize:11, color:"#7c3aed", marginTop:2 }}>Untuk pelanggan kredit — tiada diskaun</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontWeight:800, fontSize:16, color:"#6d28d9" }}>RM {calcResult.priceF.toFixed(2)}/unit</div>
                        <div style={{ fontWeight:600, fontSize:12, color:"#7c3aed" }}>RM {calcResult.creditTotal.toFixed(2)} jumlah</div>
                      </div>
                    </div>
                  )}

                  {/* Boss escalation message */}
                  <div style={{ padding:"10px 14px", background:C.accentLight, border:`1px solid #fcd5a0`, borderRadius:8, fontSize:12, color:C.accent, fontWeight:600 }}>
                    ⚠️ Harga ini adalah harga terbaik untuk {calcResult.qty} unit. Hubungi boss jika pelanggan minta lebih murah.
                  </div>
                </>
              )}
                  ...
                  <tbody>
                    {items.map((p,i)=>(
                      <tr key={p.id} style={{ background:i%2===0?C.white:C.gray, borderBottom:`1px solid ${C.border}` }}>
                        ...
                        <td style={{ padding:"8px 10px", fontWeight:800, fontSize:13, color:(p.retailPrice||p.price)>0?C.navy:"#cbd5e1" }}>{(p.retailPrice||p.price)>0?`RM ${parseFloat(p.retailPrice||p.price).toFixed(2)}`:"—"}</td>
                        <td style={{ padding:"8px 10px", fontWeight:700, fontSize:13, color:p.bulkPrice>0?C.green:"#cbd5e1" }}>{p.bulkPrice>0?`RM ${parseFloat(p.bulkPrice).toFixed(2)}`:"—"}</td>
                        <td style={{ padding:"8px 10px", fontWeight:700, fontSize:13, color:p.creditPrice>0?"#6d28d9":"#cbd5e1" }}>{p.creditPrice>0?`RM ${parseFloat(p.creditPrice).toFixed(2)}`:"—"}</td>
                    ...
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                      {(p.retailPrice||p.price)>0 && (
                        <button onClick={()=>{set("product",p.product+(p.size?" "+p.size:""));set("originalPrice",String(p.retailPrice||p.price));setPriceSearch("");}}
                          ...
                          Retail: RM {parseFloat(p.retailPrice||p.price).toFixed(2)}
                        </button>
                      )}
                      {p.bulkPrice>0 && (
                        <button onClick={()=>{set("product",p.product+(p.size?" "+p.size:""));set("originalPrice",String(p.bulkPrice));setPriceSearch("");}}
                          ...
                          Kuantiti: RM {parseFloat(p.bulkPrice).toFixed(2)}
                        </button>
                      )}
                      {p.creditPrice>0 && (
                        <button onClick={()=>{set("product",p.product+(p.size?" "+p.size:""));set("originalPrice",String(p.creditPrice));setPriceSearch("");}}
                          ...
                          Kredit: RM {parseFloat(p.creditPrice).toFixed(2)}
                        </button>
                      )}
                    </div>
                  </div>
...
// ════════════════════════════════════════════════════════════════════════════
// USERS & PIN MANAGEMENT TAB — Owner only
// ════════════════════════════════════════════════════════════════════════════
const ROLES = ["staff", "senior", "owner"];

function UsersTab({ session }) {
  const [users, setUsers]     = useState([]);
  const [loading, setLoading] = useState(true);
  const [saved, setSaved]     = useState("");
  const [editing, setEditing] = useState(null); // null | user object
  ...
  useEffect(() => {
    gsGet("getUsers").then(r => {
      if (r.success && r.users.length > 0) {
        ...
        // First time — seed from hardcoded STAFF_PINS
        setUsers(STAFF_PINS.map(s => ({ ...s, active:"yes" })));
      }
      setLoading(false);
    });
  }, []);
  ...
  const saveUser = async () => {
    ...
    let updated;
    if (editing==="new") {
      const newUser = { name:form.name, pin:form.pin, role:form.role||"staff", active:"yes" };
      updated = [...users, newUser];
    } else {
      updated = users.map(u => u.name===editing ? { ...u, ...form } : u);
    }
    setUsers(updated);
    ...
    <div>
      ...
      <Card style={{ padding:"12px 14px", marginBottom:14, background:"#f0f9ff", border:"1px solid #bae6fd" }}>
        ...
        <div style={{ fontSize:12, color:"#0369a1" }}>
          Tambah atau ubah pengguna dan PIN di sini. Perubahan akan berkuat kuasa serta-merta pada semua peranti.
        </div>
      </Card>
      ...
      {editing && (
        <Card style={{ padding:18, marginBottom:14, border:`2px solid ${C.accent}` }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
            <div>
              <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.muted, marginBottom:4, textTransform:"uppercase" }}>Nama</label>
              <input value={form.name||""} onChange={e=>setF("name",e.target.value)}
                placeholder="Nama penuh" disabled={editing!=="new"}
                style={{ width:"100%", padding:"9px 11px", borderRadius:8, border:`1.5px solid ${C.border}`, fontSize:13, fontFamily:"inherit", boxSizing:"border-box", background:editing!=="new"?"#f8fafc":C.white }} />
            </div>
            <div>
              <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.muted, marginBottom:4, textTransform:"uppercase" }}>PIN Baru</label>
              <input type="text" inputMode="numeric" value={form.pin||""} onChange={e=>setF("pin",e.target.value.replace(/\D/g,""))}
                placeholder="4-6 digit" maxLength={6}
                style={{ width:"100%", padding:"9px 11px", borderRadius:8, border:`1.5px solid ${C.border}`, fontSize:16, fontFamily:"inherit", letterSpacing:4, boxSizing:"border-box" }} />
            </div>
            <div>
              <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.muted, marginBottom:4, textTransform:"uppercase" }}>Peranan</label>
